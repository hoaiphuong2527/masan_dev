...
class ServiceRepositoryEloquent extends BaseRepository implements ServiceRepository
{
    ...
    {
        return $this->model->select('*')->orderByDesc('created_at')->withTranslation();
    }
    ...
    public function update(array $input, $id)
    {
        ...
        foreach($locales as $locale){
            if(!empty($input[$locale]['slug'])){
                ...
                DB::table('service_translation')
                    ->where('service_id', $id)->where('locale', $locale)
                    ->update(['slug' => $slug]);
            }
        }
    ...
    public function listService($limit = 0, $is_top = false)
    {
    ...
    public function topService($limit = 0, $is_top = false)
    {
        $locale = \App::getLocale();
